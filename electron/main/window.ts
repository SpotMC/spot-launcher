...
import { reportLauncherStarted } from "./stats-client"
...
let legalWindow: BrowserWindow | null = null

export function createLegalWindow() {
  if (legalWindow && !legalWindow.isDestroyed()) {
    legalWindow.focus()
    return
  }
  logRuntime("[Window] Creating legal window")
  legalWindow = new BrowserWindow({
    width: 900,
    height: 720,
    minWidth: 680,
    minHeight: 520,
    title: "Документы — Spot Launcher",
    show: false,
  ...
  if (isDev) {
    void legalWindow.loadURL("http://localhost:5173/?win=legal")
  } else {
    void legalWindow.loadFile(path.join(__dirname, "../../dist/index.html"), { query: { win: "legal" } })
  }
...
export function closeLegalWindow() {
  if (legalWindow && !legalWindow.isDestroyed()) {
    legalWindow.close()
    legalWindow = null
  }
...
export function registerWindowLifecycle() {
  ...
  ipcMain.handle("legal:open", () => createLegalWindow())
  ipcMain.on("legal:close", () => closeLegalWindow())
  ...
  app.whenReady().then(async () => {
    ...
    // Сообщить на сайт, что лаунчер запустился (fire-and-forget).
    void reportLauncherStarted()
    try {
