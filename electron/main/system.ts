...
type JavaDetection = { path: string; version: string; label: string; fullVersion?: string; vendor?: string; arch?: string }

export async function scanForJavaInstallations(): Promise<JavaDetection[]> {
  const found: JavaDetection[] = []
...
export function registerSystemHandlers() {
  ...
  ipcMain.handle("java:detect", async (): Promise<JavaDetection[]> => {
    return scanForJavaInstallations()
  })
  ...
  ipcMain.handle("logs:share-to-mclogs", async (_event, content: string): Promise<{ success: boolean; url?: string; error?: string }> => {
    try {
      const res = await fetchWithRetry("https://api.mclo.gs/1/log", {
        ...
        body: JSON.stringify({ content, source: "Spot Launcher" }),
      }, { retries: 2 })
