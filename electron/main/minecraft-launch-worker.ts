...
// ------------------------------------------------------------------
// Spot launcher: auto-close the game the moment the player leaves the
// server. Since every launch is forced into a single fixed server, any
// disconnect/leave is a signal to quit immediately, so the user drops
// straight out of the game instead of sitting on the main menu.
// ------------------------------------------------------------------
let autoQuitOnLeaveArmed = false

function armAutoQuitOnLeave(): void {
  autoQuitOnLeaveArmed = true
}

function disarmAutoQuitOnLeave(): void {
  autoQuitOnLeaveArmed = false
}

const SERVER_LEAVE_PATTERNS: RegExp[] = [
  /lost connection:/i,
  /connection lost:/i,
  /disconnected from server/i,
  /leaving the server/i,
  /disconnect requested by server/i,
]

function handleGameOutputLine(line: string): void {
  if (!autoQuitOnLeaveArmed || !minecraftProcess || minecraftProcess.killed) return
  if (!SERVER_LEAVE_PATTERNS.some((re) => re.test(line))) return

  autoQuitOnLeaveArmed = false
  debug(`[Spot] Обнаружен выход с сервера («${line.trim()}») — завершаю игру`)
...
async function launchMinecraft(payload: WorkerLaunchPayload): Promise<void> {
  ...
  // Quick Play is passed through only when explicitly requested (e.g. the
  // auto-join server option). By default nothing is forced, so a normal
  // launch opens singleplayer / the main menu.
  ...
  if (qpOptions.quickPlayMultiplayer) {
  ...
  const stdoutRelay = new OutputRelay("stdout", (msg) => {
    handleGameOutputLine(typeof msg.data === "string" ? msg.data : "")
    send(msg)
  })
