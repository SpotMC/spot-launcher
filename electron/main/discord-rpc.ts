...
// Discord-приложение «Spot Launcher»: Client ID меняется на официальное.
// Название, отображаемое под активностью в Discord, задаётся самим приложением,
// зарегистрированным под этим ID (а не кодом). Ассеты (largeImageKey/smallImageKey)
// создаются в Discord Developer Portal у этого приложения.
const CLIENT_ID = process.env.SPOT_DISCORD_CLIENT_ID || "1542538990752895048"
let rpc: any = null
let connected = false
...
async function loginWithRuntimeDir(runtimeDir?: string): Promise<any> {
  ...
  client.on("ready", () => {
    connected = true
    console.log("[DiscordRPC] ready — clientId:", CLIENT_ID)
  ...
  try {
    ...
    console.error("[DiscordRPC] client.login failed:", err)
    throw err
  } finally {
...
export async function initDiscordRpc(): Promise<void> {
  ...
  if (process.platform === "win32") {
    try {
      ...
      console.log("[DiscordRPC] подключено (clientId:", CLIENT_ID, ")")
      return
    } catch (err) {
      console.error("[DiscordRPC] не удалось подключиться (win32):", err)
    }
    return
  }
...
function applyActivity(activity: DiscordActivity): void {
  const {
    state = "В меню",
    largeImageKey = "spot-logo-mono-2",
    largeImageText = "Spot Launcher",
    smallImageKey,
    smallImageText,
    loader,
    startTimestamp,
  ...
  const activityData: Record<string, unknown> = {
    ...
    largeImageText,
    buttons: [
      { label: "Сайт", url: "https://spotmc.ru" },
      { label: "Discord Сервер", url: "https://discord.gg/ukz9kYVM9B" },
    ],
  }
...
if (smallImageKey) {
  ...
  // Маленькая иконка: используем ассет footer-logo, который есть у приложения
  // в Discord Developer Portal. Остальные иконки загрузчиков (fabric_icon и т.п.)
  // не загружены, поэтому их не используем.
  ...
  const finalSmallImageKey = smallImageKey ?? "footer-logo"
  const finalSmallImageText = loader
    ? loaderNames[loader.toLowerCase()] ?? smallImageText
    : smallImageText
