...
export function LogsPage() {
  ...
  return (
    <div className="h-[700px] flex flex-col gap-4 animate-in fade-in-0 duration-300 bg-[#0c0d10]">
      <div className="flex h-[46px] shrink-0 items-center gap-3 border-b border-white/[0.04] bg-[#0c0d10] px-4">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <h1 className="text-[14px] font-semibold text-white/90">{t("logs.title")}</h1>
        </div>
        <div className="flex-1" />
        <div className="flex items-center gap-2">
          <button type="button" onClick={handleCopy}
            className="flex items-center gap-2 px-3 py-2 rounded-xl text-sm font-medium bg-white/[0.03] border border-white/[0.08] hover:bg-white/[0.07] text-white/60 hover:text-white transition-colors">
          ...
          <button type="button" onClick={clearLogs}
            className="flex items-center gap-2 px-3 py-2 rounded-xl text-sm font-medium bg-white/[0.03] border border-white/[0.08] hover:bg-destructive/20 text-white/60 hover:text-destructive transition-colors">
      ...
      {/* Filters + Search */}
      <div className="flex items-center gap-3 flex-wrap flex-shrink-0 px-4">
        <div className="flex items-center gap-1 p-1 rounded-xl bg-white/[0.03] border border-white/[0.06] flex-shrink-0">
          {FILTER_DEFS.map(({ id, icon: Icon }) => (
            <button key={id} type="button" onClick={() => setFilter(id)}
              className={cn("flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all border",
                filter === id ? "border-transparent bg-[#5a6ff2] text-white" : "border-transparent bg-transparent text-white/40 hover:text-white/70 hover:bg-white/[0.05]")}>
        ...
        <div className="relative flex-1 min-w-[180px]">
          ...
          <input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder={t("logs.search")}
            className="w-full pl-9 pr-4 py-2 rounded-xl bg-white/[0.03] border border-white/[0.08] text-sm text-white placeholder:text-white/25 focus:outline-none focus:border-[#5a6ff2]/60" />
        </div>
        ...
        <button type="button" onClick={() => setAutoScroll(v => !v)}
          className={cn("flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-medium transition-colors flex-shrink-0",
            autoScroll ? "bg-[#5a6ff2]/20 text-[#8d9ff5]" : "bg-white/[0.03] border border-white/[0.08] text-white/40 hover:text-white/70 hover:bg-white/[0.07]")}>
