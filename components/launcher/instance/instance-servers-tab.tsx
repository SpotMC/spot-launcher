...
// The official Spot server. It is always shown in the server list, but its
// address is hidden from the player — connection happens against this address.
const SPOT_SERVER = {
  name: "SpotMC",
  ip: "185.9.145.192",
  port: 30716,
...
function isSpotServer(ip: string): boolean {
  return ip.split(":")[0].toLowerCase() === SPOT_SERVER.ip
}
...
export function InstanceServersTab({ build, updateBuild }: InstanceServersTabProps) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      // Always ensure the official Spot server is present in the list.
      if (!serverList.some((s) => isSpotServer(s.ip))) {
        serverList = [{ name: SPOT_SERVER.name, ip: SPOT_SERVER.ip }, ...serverList]
      }
      ...
      const list = serverList.map((s) => ({
        ...
        loading: true,
        error: null,
        isFavorite: isSpotServer(s.ip) || favoriteIps.has(s.ip),
        protected: isSpotServer(s.ip),
      }))
  ...
  const favoriteServers = filtered.filter((s) => s.isFavorite || s.protected)
  const onlineServers = filtered.filter((s) => s.status?.online && !s.isFavorite && !s.protected)
  const offlineServers = filtered.filter((s) => !s.status?.online && !s.isFavorite && !s.protected)

  const toggleFavorite = (ip: string) => {
    // Закреплённый сервер Spot нельзя открепить.
    if (isSpotServer(ip)) return
  ...
  const renderServer = (server: ServerEntry, index: number) => {
    ...
    const isProtected = server.protected === true
    ...
    return (
      ...
      >
        ...
        <div className="flex items-center gap-2 flex-shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
          ...
          <button
            onClick={() => { if (!isProtected) setServers((prev) => prev.filter((p) => p.ip !== server.ip)) }}
            disabled={isProtected}
            className="p-2 rounded-lg bg-muted/50 hover:bg-destructive/20 text-muted-foreground hover:text-destructive transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
            title={isProtected ? t("servers.protected") : undefined}
          >
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 rounded-xl bg-primary hover:bg-primary/90 text-primary-foreground font-medium transition-all duration-200"
              >
