...
import { IconCamera, IconTrash, IconExternalLink, IconFolderOpen, IconPlayerPlay } from "@tabler/icons-react"
...
export function InstanceDetailGeneral({ activeBuild, updateBuild, renameBuild, onLaunch }: InstanceDetailGeneralProps) {
  ...
  return (
    <div className="flex-1 overflow-y-auto">
      <div className="mb-5 flex items-center justify-between gap-4 rounded-3xl border border-border bg-card/40 p-4">
        <div className="min-w-0">
          <div className="text-sm text-muted-foreground">Готова к запуску</div>
          <div className="truncate text-lg font-semibold text-foreground">{activeBuild.name || "Новая сборка"}</div>
        </div>
        <button
          type="button"
          onClick={onLaunch}
          className="flex flex-shrink-0 items-center justify-center gap-2 rounded-2xl bg-[#5a6ff2] px-8 py-3.5 text-base font-bold text-white transition-colors hover:bg-[#6c7ff6]"
        >
          <IconPlayerPlay className="h-5 w-5" strokeWidth={2} />
          ИГРАТЬ
        </button>
      </div>
